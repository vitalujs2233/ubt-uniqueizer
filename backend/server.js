...
app.post('/dvizh/create-post', async (req, res) => {
  try {
    ...
    if (e.message === 'Недостаточно средств' || e.message === 'Пользователь не найден') {
...
app.post('/dvizh/toggle-like', async (req, res) => {
  try {
    ...
    const postResult = await pool.query(
      `select likes_count from posts where id = $1 limit 1`,
      [postId]
    );
...
app.post('/dvizh/add-view', async (req, res) => {
  try {
    ...
    const postResult = await pool.query(
      `select views_count from posts where id = $1 limit 1`,
      [postId]
    );

    res.json({
      ok: true,
      viewsCount: postResult.rows[0]?.views_count || 0
    });
  } catch (e) {
    console.error(e);
    res.status(500).json({ message: 'Server error' });
